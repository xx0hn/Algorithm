...
for _ in range(m):
    ...
    client.append((y1-1, x1-1, y2-1, x2-1))
client.sort(key=lambda x:(x[0], x[1]))
dy, dx=[0, 1, 0, -1], [1, 0, -1, 0]
def find_way(sy, sx):
    q=deque()
    q.append((sy, sx, 0))
    dists=[[1e9 for _ in range(n)] for _ in range(n)]
    dists[sy][sx]=0
    while q:
        y, x, d=q.popleft()
        if dists[y][x]<d:
        ...
        for i in range(4):
            ny, nx=y+dy[i], x+dx[i]
            if 0<=ny<n and 0<=nx<n and dists[ny][nx]>dists[y][x]+1 and grid[ny][nx]==0:
                dists[ny][nx]=dists[y][x]+1
                q.append((ny, nx, d+1))
    return dists
while client:
    dist=find_way(taxi[0], taxi[1])
    d=1e9
    idx=0
    for i in range(len(client)):
        if d>dist[client[i][0]][client[i][1]]:
            d=dist[client[i][0]][client[i][1]]
            idx=i
        if dist[client[i][0]][client[i][1]]==1e9:
            print(-1)
            quit()
    oil-=d
    if oil<=0:
        print(-1)
        quit()
    taxi=[client[idx][0], client[idx][1]]
    dist1=find_way(taxi[0], taxi[1])
    oil-=dist1[client[idx][2]][client[idx][3]]
    if oil<0:
        print(-1)
        quit()
    taxi=[client[idx][2], client[idx][3]]
    oil+=(dist1[client[idx][2]][client[idx][3]]*2)
    client.pop(idx)
print(oil)
